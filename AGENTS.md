...
5. **Act 收尾必落库（2026-08-14 用户定）**：进入 Act 阶段（收尾）时，把本轮**全部经验教训和知识**写入知识库——bug 根因 → `references/4_bugs/` 对应子层；经验/流程/决策 → `references/5_sessions/` 会话归档（或对应层）；按知识库入库流程（草稿 → reviewer 审查 → 合并 → 更新 README 索引）。**落库是 Act 的固定动作，不是可选**。
