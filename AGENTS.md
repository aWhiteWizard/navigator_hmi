...
- **铁律一（重要结论及时落盘）**：每轮 Do 完成 → 更新 PROJECT.md（继续点/提交记录/测试数）；每个 bug 根因 → 知识库 4_bugs；**可复用经验/流程/方法 → 对应长期知识层（1_application/2_system/6_content 等）；5_sessions 仅作临时会话记录（会删），不是知识存放处**。**对话里产生的关键状态必须在压缩前落到磁盘**，不依赖会话回忆。
...
3. **⚠️ 例外：代码评论 = 修改授权（2026-08-30 用户定，新劳动纪律生效起始）**：Check 阶段用户通过 **Git Review 插件**在「改动」tab 提的**代码评论（comments）**，是用户对代码的具体修改意见——收到评论即视为对该评论点位的**修改授权**，主 Agent 应直接修改对应代码并在评论下回复方案/结果（等同返回上一轮 Do 处理该评论点位）；评论闭环后照常跑编译+测试+reviewer 审查。**注意边界**：只有「代码评论」触发此例外；用户在对话里口头说的修改意见仍按「只记录不实施」处理。
4. Check 期间不得主动提议「进入下一轮 Do」或任何环节切换（评论触发的修改例外不受此限）。
5. **Act 收尾必落库（2026-08-14 用户定；2026-09-02 强化：不只 bug 还有经验，5_sessions 不放知识）**：进入 Act 阶段（收尾）时，把本轮**全部经验教训和知识**写入能力仓库，不要只写 bug——**成功流程/方法/决策 → 对应长期知识层**（bug 根因 → `references/4_bugs/` 对应子层；应用/系统/内容等经验 → `references/1_application/2_system/6_content` 等对应层）；**用过的脚本 → `references/7_scripts/`**；**5_sessions 仅作临时会话记录（cleanup 会删），可复用知识必须总结提炼进长期层，不能只放 5_sessions**；按知识库入库流程（草稿 → reviewer 审查 → 合并 → 更新 README 索引）。**落库是 Act 的固定动作，不是可选**。
6. **Act→Plan 知识库提交（2026-08-14 用户定）**：离开 Act、进入下一阶段 Plan 时，**把本轮落库的知识库更新提交到 Git**（`~/.pi/agent/skills/` 仓库，含 4_bugs/5_sessions 及 README 索引变更）——commit message 无 BOM；是否 push 按用户批准。**提交是 Act 转 Plan 的固定动作**，确保知识库变更落盘可追溯，不随会话丢失。
7. **开工前阶段校验（2026-08-14 用户强调，防"方向 ≠ Do"误判）**：任何实施动作（写文件/改代码/建目录/提交/push）之前，先核对：① PROJECT.md「继续点」的当前阶段；② 用户是否说了**明确的环节切换短语**（如「当前阶段结束，进入下一阶段」「开始执行」「进入 Do」）。**只有两者都满足才实施**；否则任何指示一律视为「方向 / Plan 定稿授权」，只记录、不实施。拿不准时停下来问，绝不猜。用户强调：**只有用户明确说「当前阶段结束，进入下一阶段」才是正确的环节切换**——「放 D 盘」「都加入」「你确定好就行」等均不等于进入 Do。**代码评论例外不受此限**（评论即明确的修改授权）。
8. **🔴 环节外禁止实施（2026-08-30 用户强调，防漏改错改；Act 阶段越权实录修正）**：**只有 ① 当前环节是 Do、或 ② 收到 Git Review 代码评论** 才允许改代码/写文件/提交。**Check 阶段对话反馈**（含「修吧」「这个不对」「这个有问题」）一律**只记录不实施**——问题记入 PROJECT.md「Check 环节记录续N」区并标注「待下一轮 Do」，绝不擅自实施。**Act 阶段是收尾落库，同样禁止任何代码修改**（Act=落库/删执行书/甘特图/提交知识库，不做 Do 的事）。**违反后果**：① 未授权实施会把「待办问题」误标成「已修复」，掩盖真实状态（漏改/错改）——问题被当作处理过、下一轮不再看，实际用户没确认、也没进 Do（2026-08-30 实录：N+17 设备操作不标脏在 Act 阶段越权实施，误写「用户明确返回 Do」，用户指出后修正为待下一轮 Do）；② 混入不该做的改动，造成 diff 混乱。**问题状态以 PROJECT.md 记录为准，实施必须等用户明确环节切换**；拿不准时停下来问，绝不猜。
